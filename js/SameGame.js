...
(function() {
  window.app = {
    config: function() {
      this.define.x_axis = Number($('#x_axis').val());
      this.define.y_axis = Number($('#y_axis').val());
      return this.define.num = Number($('#num').val());
    },
    define: {
      x_axis: 15,
      y_axis: 15,
    ...
    createlist: function() {
      ...
      for (i = k = 1, ref = this.define.x_axis; 1 <= ref ? k < ref : k > ref; i = 1 <= ref ? ++k : --k) {
        $('#x_axis').append("<option value=\"" + i + "\">" + i + "</option>");
      }
      for (j = l = 1, ref1 = this.define.y_axis; 1 <= ref1 ? l < ref1 : l > ref1; j = 1 <= ref1 ? ++l : --l) {
        $('#y_axis').append("<option value=\"" + j + "\">" + j + "</option>");
      }
      for (n = m = 2, ref2 = this.define.num; 2 <= ref2 ? m < ref2 : m > ref2; n = 2 <= ref2 ? ++m : --m) {
    ...
    del: function(delarray, cparray) {
      var cpx, cpy, fixy, i, j, k, l, len, m, o, ref, ref1, ref2, results, x, xpos, y, ypos;
      ...
      results = [];
      for (i = o = 0, len = cparray.length; o < len; i = ++o) {
        cpx = cparray[i];
        results.push((function() {
          var len1, p, results1;
          results1 = [];
          for (j = p = 0, len1 = cpx.length; p < len1; j = ++p) {
            cpy = cpx[j];
